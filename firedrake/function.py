...
def make_c_evaluate(function, c_name="evaluate", ldargs=None):
    ...
    src = pq_utils.src_locate_cell(mesh)
    src += compile_element(function)

    args = []

    arg = mesh.coordinates.dat(op2.READ, mesh.coordinates.cell_node_map(), flatten=True)
    arg.position = 0
    args.append(arg)

    arg = function.dat(op2.READ, function.cell_node_map(), flatten=True)
    arg.position = 1
    args.append(arg)
